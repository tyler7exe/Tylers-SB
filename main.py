...
try:
   set_terminal_title("| Tylers-SB Selfbot |")
...
if check_loop:
    try:
        ...
        log.warning(f"Error while trying to check the last  version: {e}")
...
async def on_ready():
    ...
    if rpc.read_variable_json("create_panel"):
        try:
            ...
            await panel.edit(name=" Tylers-SB Panel", icon=_icon)
            ...
            log.warning('Icon file not found, skipping panel creation')
            rpc.edit_variable_json('create_panel', False)
        log.alert("Panel successfully created (check DMs!).\nIf not, please check the 'Issues' category into the GitHub's README for further help.")
...
async def on_message(message):
    ...
    try:
        if hasattr(bot, 'exe_responses') and message.author.id in bot.exe_responses:
            messages_list = bot.exe_responses[message.author.id]
    ...
    try:
        log.info(f"on_message: author={message.author} id={message.author.id} content={getattr(message,'content','')[:150]}")
    except Exception:
        pass

    if hasattr(bot, 'exe_responses'):
        log.info(f"exe_responses keys: {list(bot.exe_responses.keys())}")

    if hasattr(bot, 'exe_responses') and message.author.id in bot.exe_responses:
        random_message = random.choice(bot.exe_responses[message.author.id])
        try:
            await message.channel.send(random_message)
            log.success(f"Sent exe auto-response to {message.author.id}")
        except Exception as e:
            log.error(f"Error sending exe response: {e}")
